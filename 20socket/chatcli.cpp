...
void parse_cmd(char *cmdline, int sock, struct sockaddr_in *servaddr)
{
	...
	if(strcmp(cmd, "exit") == 0)
	{
		...
		memset(&msg, 0, sizeof(msg));
        	...
        	if(sendto(sock, (const char*)&msg, sizeof(msg), 0, (struct sockaddr*)servaddr, sizeof(*servaddr)) < 0)
			ERR_EXIT("sendto");
...
void chat_cli(int sock)
{
	...
	while(1)	/*输入用户名，进行登录*/
	{
		...
		if(cmd == S2C_ALREADY_LOGINED)	/*已有同名用户登录，重新读取用户名*/
	...
	while(1)
	{
		...
		{
			char cmdline[100] = {0};
			if(fgets(cmdline, sizeof(cmdline), stdin) == NULL) /*输入ctrl+D，读到EOF，用户登出*/
				break;
